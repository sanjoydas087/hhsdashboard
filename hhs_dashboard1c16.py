...
/* =========================================================
   HHS EXECUTIVE SUMMARY — GOVERNMENT OPERATIONS PANEL
   ========================================================= */

.hhs-executive-summary {
    position: relative;
    background: #112E51;
    border: 1px solid #0B1F38;
    border-top: 4px solid #C5A34E;
    border-radius: 0 0 12px 12px;
    padding: 1.45rem 1.55rem 1.35rem 1.55rem;
    margin: 1.25rem 0 1.5rem 0;
    color: #FFFFFF;
    box-shadow: 0 8px 24px rgba(17,46,81,0.16);
    overflow: hidden;
}

/* subtle background architecture */
.hhs-executive-summary::before {
    content: "";
    position: absolute;
    width: 180px;
    height: 180px;
...
.hes-narrative {
    ...
    background: rgba(32,84,147,0.42);
...
.hes-action {
    ...
    background: rgba(197,163,78,0.13);
    border: 1px solid rgba(197,163,78,0.42);
    border-radius: 6px;
    position: relative;
    z-index: 2;
}
...
def load_and_train():
    ...
    results.append(
        evaluate_model("SARIMA(1,1,1)(1,1,1,7)", y_test.values, sarima_p.values)
    )
    ...
    for name, rmses in cv_rmse.items():
        ...
        robustness[name] = {
            ...
            "score": max(
                0.0, round(100 - cov_pct, 1)
            ),  # 100 = perfectly stable across folds
        }
    ...
    return (
        ...
        y_test.index.strftime("%Y-%m-%d").tolist(),
        rf,
        gb,
        FEATURES,
        robustness,
...
def forecast_future_rf(
    model, df, FEATURES, n_days, transfer_mult=1.0, discharge_mult=1.0
):
    ...
    if all(
        c in last_row.index
        for c in ["Net_Pressure", "Transfers_to_HHS", "HHS_Discharged"]
    ):
        last_row["Net_Pressure"] = (
            last_row["Transfers_to_HHS"] - last_row["HHS_Discharged"]
        )
...
def hhs_executive_summary(
    ...
    if net_balance_30 > 10:
        ...
        pressure_value_class = "red"
        pressure_icon = "⬆"
    elif net_balance_30 < -10:
        pressure_class = "success"
        pressure_value_class = "green"
        pressure_icon = "⬇"
    else:
        pressure_class = "info"
    ...
    if breach_pct > 30:
        breach_class = "danger"
        breach_value_class = "red"
    elif breach_pct > 0:
        breach_class = "warning"
        breach_value_class = "amber"
    else:
        breach_class = "success"
        breach_value_class = "green"

    if surge_days is None:
        surge_text = "No projected breach"
        surge_value_class = "green"
    else:
        surge_text = f"Day {surge_days + 1}"
        surge_value_class = "red" if surge_days <= 21 else "amber"

    # ── Planning message ──
    if surge_days is not None and surge_days <= 7:
        planning_text = (
            f"Capacity pressure is projected within the next "
            f"<strong>{surge_days + 1} day(s)</strong>. "
            f"Review placement capacity, staffing and operational readiness "
            f"against the <strong>{capacity_threshold:,}-child</strong> threshold."
        )

    elif surge_days is not None and surge_days <= 21:
        planning_text = (
            f"A capacity threshold crossing is projected on "
            f"<strong>Day {surge_days + 1}</strong>. "
            f"Use the available lead time to review staffing, shelter capacity "
            f"and placement resources."
        )

    elif net_balance_30 > 10:
        planning_text = (
            "Recent intake is running above discharge activity. "
            "Continue monitoring net pressure and review placement capacity "
            "if the intake surplus persists."
        )

    else:
        planning_text = (
            "Continue routine monitoring of care load, intake and discharge "
            "patterns. Reassess capacity if the forecast or net pressure "
            "indicator changes materially."
        )

    st.markdown(
        f"""
        <div class="hhs-executive-summary">
            <div class="hes-header">
                <div class="hes-icon">📊</div>
                <div class="hes-title">
                    Executive Summary
                </div>
                <div class="hes-subtitle">
                    Care Load & Placement Intelligence
                </div>
            </div>
            <div class="hes-grid">
                <div class="hes-item">
                    <div class="hes-label">Current HHS Care Load</div>
                    <div class="hes-value">
                        {current_care:,}
                        <span class="muted"> children</span>
                    </div>
                </div>
                <div class="hes-item {pressure_class}">
                    <div class="hes-label">30-Day Net System Pressure</div>
                    <div class="hes-value {pressure_value_class}">
                        {pressure_icon} {net_balance_30:+,.1f}
                        <span class="muted"> children/day · {pressure_word}</span>
                    </div>
                </div>
                <div class="hes-item info">
                    <div class="hes-label">30-Day Intake vs Discharge</div>
                    <div class="hes-value">
                        {avg_transfer_30:,.1f}
                        <span class="muted"> transfers/day</span>
                        &nbsp;·&nbsp;
                        {avg_discharge_30:,.1f}
                        <span class="muted"> discharges/day</span>
                    </div>
                </div>
                <div class="hes-item">
                    <div class="hes-label">Forecast Average</div>
                    <div class="hes-value">
                        {forecast_avg:,}
                        <span class="muted"> children/day · next {horizon} days</span>
                    </div>
                </div>
                <div class="hes-item {'danger' if forecast_peak > capacity_threshold else 'success'}">
                    <div class="hes-label">Peak Forecast</div>
                    <div class="hes-value {'red' if forecast_peak > capacity_threshold else 'green'}">
                        {forecast_peak:,}
                        <span class="muted"> children</span>
                    </div>
                </div>
                <div class="hes-item {breach_class}">
                    <div class="hes-label">Capacity Exposure</div>
                    <div class="hes-value {breach_value_class}">
                        {breach_days} / {horizon}
                        <span class="muted"> days above threshold · {breach_pct}%</span>
                    </div>
                </div>
                <div class="hes-item {'danger' if surge_days is not None else 'success'}">
                    <div class="hes-label">Projected Surge Timing</div>
                    <div class="hes-value {surge_value_class}">
                        {surge_text}
                    </div>
                </div>
                <div class="hes-item info">
                    <div class="hes-label">Forecast Model</div>
                    <div class="hes-value">
                        {model_name}
                        <span class="muted"> · stability {stability_index:.4f}</span>
                    </div>
                </div>
            </div>
            <div class="hes-narrative">
                <strong>System Outlook:</strong>
                The current care load is <strong>{current_care:,}</strong> children.
                Over the last 30 days, HHS received an average of
                <strong>{avg_transfer_30:,.1f}</strong> transfers per day and recorded
                <strong>{avg_discharge_30:,.1f}</strong> discharges per day,
                producing a net pressure of
                <strong>{net_balance_30:+,.1f} children/day</strong>.
                The selected forecast projects an average daily care load of
                <strong>{forecast_avg:,}</strong> children over the next
                <strong>{horizon} days</strong>.
            </div>
            <div class="hes-action">
                <div class="hes-action-title">
                    💡 Planning Focus
                </div>
                <div class="hes-action-text">
                    {planning_text}
                </div>
            </div>
        </div>
...
<div class="dash-hero">
        ...
        <h1>Predictive Forecasting of Care Load & Placement Demand Dashboard</h1>
...
show_historical_window = st.sidebar.slider(
    "Historical Window (Days)", min_value=30, max_value=365, value=180
)
...
# ── KPI values ──
surge_days = next(
    (i for i, v in enumerate(future_forecast) if v > capacity_threshold), None
)
...
with tab1:
    ...
    t1_trend = (
        "Increasing ⬆"
        if future_forecast.iloc[-1] > future_forecast.iloc[0]
        else "Decreasing ⬇"
    )
    t1_trend_color = (
        "amber" if future_forecast.iloc[-1] > future_forecast.iloc[0] else "green"
    )
    ...
    with t1c1:
        kpi_card(
            f"Forecast Avg (Next {horizon}d)",
            f"{t1_avg:,}",
            "Mean projected <br>daily care load",
            "navy",
        )
    with t1c2:
        kpi_card(
    ...
    with t1c3:
        kpi_card(
            "Trend Direction",
            t1_trend,
            "First vs. last <br>day of forecast window",
            t1_trend_color,
        )
    with t1c4:
        kpi_card(
            "Model Robustness",
            f"{rf_robustness['score']}",
    ...
    if show_confidence:
        fig1.add_trace(
            go.Scatter(
                ...
                fill="toself",
                fillcolor="rgba(184,134,11,0.12)",
    ...
    fig1.update_layout(
        title=dict(
            ...
            font=dict(size=14, color="#112E51", family="Inter"),
            x=0.01,
        ),
    ...
    chart_row(
        ...
        [
            ...
            (
                "Surge Day",
                f"Day {surge_days}" if surge_days is not None else "No Breach",
                None,
            ),
        ],
    ...
    try:
        ...
        fig_decomp.add_trace(
            go.Scatter(
                ...
                y=decomp.observed,
                name="Observed",
                line=dict(color="#3D4551", width=1.4),
            ),
            row=1,
            col=1,
        )
        fig_decomp.add_trace(
            go.Scatter(
                ...
                y=decomp.trend,
                name="Trend",
                line=dict(color="#205493", width=2),
            ),
            row=2,
            col=1,
        )
        fig_decomp.add_trace(
            go.Scatter(
                ...
                y=decomp.seasonal,
                name="Seasonal",
                line=dict(color="#5B3A8E", width=1.3),
            ),
            row=3,
            col=1,
        )
        ...
        resid_share = (
            float(np.std(resid_series) / np.std(decomp.observed.dropna())) * 100
        )
        trend_range = f"{int(decomp.trend.min()):,} – {int(decomp.trend.max()):,}"
        seasonal_amp = float(decomp.seasonal.max() - decomp.seasonal.min())
        resid_quality = (
            "Clean (near-zero, no pattern)"
            if resid_share < 15
            else "Elevated — check for unmodeled effects"
        )
    ...
    forecast_table["Status"] = forecast_table["Forecast"].apply(
        lambda x: (
            "🔴 Above Threshold" if x > capacity_threshold else "🟢 Within Capacity"
        )
    )
    ...
    ci_half_day1 = int(
        (forecast_table["Upper CI"].iloc[0] - forecast_table["Lower CI"].iloc[0]) / 2
    )
    ci_half_day14 = int(
        (forecast_table["Upper CI"].iloc[-1] - forecast_table["Lower CI"].iloc[-1]) / 2
    )
    ...
    # ── Scenario Comparison ──
    section_header(
        "🧭", "Scenario Comparison — All Forecast Paths", tag="What-If Analysis"
    )
    ...
    fig_scenario.update_layout(
        title=dict(
            ...
            font=dict(size=14, color="#112E51"),
            x=0.01,
        )
    )
    ...
    chart_row(
        ...
        [
            ...
            (
                f"Day {horizon} Forecast (Active)",
                f"{int(future_forecast.iloc[-1]):,}",
                None,
            ),
    ...
    for name, series in scenario_forecasts.items():
        s_surge = next(
            (i for i, v in enumerate(series) if v > capacity_threshold), None
        )
        ...
        scenario_rows.append(
            {
                ...
                "Surge Day": (
                    f"Day {s_surge + 1}" if s_surge is not None else "No Breach"
                ),
...
with tab2:
    ...
    if net_balance_30 > 10:
        ...
        pressure_action = (
            "Discharge capacity must be increased to offset the intake surplus."
        )
    elif net_balance_30 < -10:
        pressure_word, pressure_icon = "easing", "⬇"
        pressure_action = (
            "Discharge rate currently exceeds intake — care load is declining."
        )
    else:
        pressure_word, pressure_icon = "balanced", "➡"
        pressure_action = (
            "Intake and discharge are broadly balanced — stable care load."
        )
    ...
    fig2.update_layout(
        title=dict(
            text="Daily Discharges vs Transfers In (Historical)",
            font=dict(size=14, color="#112E51"),
            x=0.01,
        )
    )
    ...
    chart_row(
        ...
        [
            (
                "Avg Daily Discharges (30d)",
                f"{avg_discharge_30}",
                "Children placed per day",
            ),
            (
                "Avg Daily Transfers In (30d)",
                f"{avg_transfer_30}",
                "Children entering daily",
            ),
            (
                "Net Daily Balance (30d)",
                f"{net_balance_30:+.1f}",
                "Positive = building",
            ),
    ...
    section_header(
        "🔮",
        "Future Intake vs Discharge Forecast",
        f"Next {horizon} Days · Holt-Winters",
    )
    ...
    fig_dfc.add_trace(
        go.Scatter(
            ...
            line=dict(color="#2E7D46", width=2),
        )
    )
    fig_dfc.add_trace(
        go.Scatter(
            ...
            line=dict(color="#2E7D46", width=2.5, dash="dash"),
        )
    )
    fig_dfc.add_trace(
        go.Scatter(
            x=df.index[-show_historical_window:],
            y=df["Transfers_to_HHS"].iloc[-show_historical_window:],
            name="Historical Transfers In",
            line=dict(color="#B31942", width=1.5, dash="dot"),
            opacity=0.6,
        )
    )
    fig_dfc.add_trace(
        go.Scatter(
            x=transfer_forecast.index,
            y=transfer_forecast.values,
            name="Forecasted Transfers In",
    ...
    fig_dfc.update_layout(
        title=dict(
            text=f"Discharge & Transfer Forecast — Next {horizon} Days",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        )
    )
    ...
    chart_row(
        ...
        [
            (
                f"Forecasted Avg Discharges ({horizon}d)",
                f"{fc_avg_discharge}",
                "Holt-Winters",
            ),
            (
                f"Forecasted Avg Transfers ({horizon}d)",
                f"{fc_avg_transfer}",
                "Holt-Winters",
            ),
    ...
    section_header(
        "⚖",
        "Future Net Pressure (Intake–Discharge)",
        f"Next {horizon} Days · Holt-Winters",
    )

    fig_fnp = go.Figure()
    fig_fnp.add_bar(
        x=future_net_pressure.index,
        y=future_net_pressure.values,
        name="Forecasted Net Pressure",
        marker_color=[
            "#B31942" if v > 0 else "#2E7D46" for v in future_net_pressure.values
        ],
    )
    ...
    fig_fnp.update_layout(
        title=dict(
            text=f"Forecasted Transfers − Forecasted Discharges (Next {horizon} Days)",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        )
    )
    ...
    imbalance_outlook = (
        "Building ⬆"
        if fc_net_avg > 0
        else ("Easing ⬇" if fc_net_avg < 0 else "Balanced ➡")
    )
    ...
    chart_row(
        ...
        [
            ...
            (
                "Days Building (of window)",
                f"{fc_days_building} / {horizon}",
                "Intake > Discharge",
            ),
    ...
    fig_np.add_bar(
        ...
        marker_color=[
            "#DC2626" if v > 0 else "#059669" for v in df["Net_Pressure"].iloc[-90:]
        ],
    )
    fig_np.add_trace(
        go.Scatter(
    ...
    fig_np.update_layout(
        title=dict(
            text="Net Pressure: Transfers − Discharges (Last 90 Days)",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        )
    )
    ...
    fig_monthly.update_layout(
        barmode="group",
        title=dict(
            text="Monthly Average: Discharges vs Transfers In",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        ),
    )

    peak_discharge_month = monthly.loc[
        monthly["Avg_Discharged"].idxmax(), "Date"
    ].strftime("%B %Y")
    peak_transfer_month = monthly.loc[
        monthly["Avg_Transfers"].idxmax(), "Date"
    ].strftime("%B %Y")
    ...
    chart_row(
        ...
        [
            ...
            (
                "Avg Monthly Discharges",
                f"{monthly['Avg_Discharged'].mean():.1f}",
                "Daily rate",
            ),
            (
                "Avg Monthly Transfers",
                f"{monthly['Avg_Transfers'].mean():.1f}",
                "Daily rate",
            ),
        ],
    ...
    with d3:
        ...
        kpi_card(
            "Net Daily Balance (30d)",
            f"{net_balance_30:+.1f}",
            "Positive = care load building",
            color,
        )
    with d4:
        kpi_card(
            "Peak Discharge (All-Time)",
            f"{max_discharge:,}",
            "Maximum single-day placements",
            "purple",
        )
...
with tab3:
    ...
    fig_rmse.update_layout(
        title=dict(
            text="Root Mean Square Error by Model",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        ),
        xaxis_tickangle=-20,
    ...
    chart_row(
        ...
        [
            ...
            (
                "Highest RMSE",
                worst_rmse_row["Model"],
                f"RMSE: {worst_rmse_row['RMSE']}",
            ),
            (
                "RMSE Range",
                f"{results_df['RMSE'].min():.1f} – {results_df['RMSE'].max():.1f}",
                None,
            ),
            (
                "Median RMSE",
                f"{results_df['RMSE'].median():.1f}",
                "Across all 7 models",
            ),
        ],
        "📋 RMSE Findings",
    ...
    fig_acc.update_layout(
        title=dict(
            text="Forecast Accuracy (%) by Model",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        ),
        xaxis_tickangle=-20,
    ...
    chart_row(
        ...
        [
            (
                "Highest Accuracy",
                best_acc_row["Model"],
                f"{best_acc_row['Accuracy (%)']}%",
            ),
            (
                "Lowest Accuracy",
                worst_acc_row["Model"],
                f"{worst_acc_row['Accuracy (%)']}%",
            ),
            (
                "Accuracy Range",
                f"{results_df['Accuracy (%)'].min():.1f}% – {results_df['Accuracy (%)'].max():.1f}%",
                None,
            ),
            (
                "Median Accuracy",
    ...
    section_header(
        "📈",
        "Actual vs Predicted — Test Period (Last 60 Days)",
        "Walk-Forward Validation",
    )

    selected_test_model = st.selectbox(
        "Select Model to Compare", options=list(test_preds.keys()), index=0
    )
    ...
    fig_avp.add_trace(
        go.Scatter(
            x=test_dates,
            y=test_preds[selected_test_model],
    ...
    sel_rmse = float(
        np.sqrt(mean_squared_error(test_actual, test_preds[selected_test_model]))
    )
    sel_maxerr = float(
        np.max(
            np.abs(np.array(test_actual) - np.array(test_preds[selected_test_model]))
        )
    )
    ...
    chart_row(
        ...
        [
            ...
            (
                "Max Single-Day Error",
                f"{sel_maxerr:,.0f}",
                "Worst-case miss in test window",
            ),
        ],
        "📋 Validation Findings",
...
with tab4:
    ...
    section_header(
        "📈", "Care Load Forecast vs Capacity Threshold", f"{horizon}-Day View"
    )
    ...
    fig4.update_layout(
        title=dict(
            text="Care Load Forecast — Capacity Breach Monitoring",
            font=dict(size=14, color="#112E51"),
            x=0.01,
        )
    )
    ...
    chart_row(
        ...
        [
            ...
            (
                "Breach Probability",
                f"{breach_pct}%",
                "% of forecast days over threshold",
            ),
        ],
    ...
    fig_roll.update_layout(
        title=dict(
            text="14-Day Rolling Average — Care Load",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        )
    )
    ...
    current_trend = (
        "declining"
        if df["HHS_Care"].iloc[-7:].is_monotonic_decreasing
        else "stable" if abs(df["HHS_Care"].iloc[-7:].diff().mean()) < 10 else "rising"
    )
    ...
    chart_row(
        ...
        [
            ...
            (
                "180-Day Range",
                f"{roll14_180_min:,.0f} – {roll14_180_max:,.0f}",
                "Rolling avg min/max",
            ),
    ...
    fig_flow.update_layout(
        title=dict(
            text="Intake Pipeline — Apprehension → Transfer → Discharge",
            font=dict(size=13, color="#112E51"),
            x=0.01,
        )
    )

    latest_apprehended = float(
        df["CBP_Apprehended"].iloc[-90:].rolling(7).mean().iloc[-1]
    )
    latest_transfers = float(
        df["Transfers_to_HHS"].iloc[-90:].rolling(7).mean().iloc[-1]
    )
    latest_discharged = float(
        df["HHS_Discharged"].iloc[-90:].rolling(7).mean().iloc[-1]
    )
...
with tab5:
    ...
    section_header(
        "🎯", "Confidence & Uncertainty Analysis", tag="Forecast Reliability"
    )
    ...
    with kc1:
        kpi_card(
            "Model Residual Std Dev",
            f"±{resid_std:,.0f}",
            "From 60-day held-out test errors (Random Forest)",
            "navy",
        )
    with kc2:
        kpi_card(
            f"Day 1 Interval Width (68%)",
            f"±{half_68[0]:,.0f}",
            "Narrowest — near-term forecasts are most reliable",
            "green",
        )
    with kc3:
        kpi_card(
            f"Day {horizon} Interval Width (68%)",
            f"±{half_68[-1]:,.0f}",
            "Widest — long-horizon uncertainty compounds",
            "amber",
        )
    with kc4:
        ...
        kpi_card(
            "Uncertainty Growth",
            f"{widening_x}×",
            f"Interval widens {widening_x}× from<br> Day 1 to Day {horizon}",
            "purple",
        )
    ...
    # ── Fan chart ──
    section_header(
        "📉", "Forecast Fan Chart — Expanding Prediction Interval", tag="68% / 95%"
    )
    ...
    fig_fan.add_trace(
        go.Scatter(
            ...
            y=future_forecast.values,
            mode="lines",
            name="Point Forecast (Random Forest)",
            line=dict(color="#205493", width=2.5),
        )
    )
    fig_fan.add_hline(
    ...
    fig_fan.update_layout(
        title=dict(
            text="Uncertainty Compounds With Forecast Horizon",
            font=dict(size=14, color="#112E51"),
            x=0.01,
        )
    )
    ...
    section_header(
        "⚖",
        "Heuristic Band vs. Statistical Fan — Method Comparison",
        tag="Transparency",
    )
    ...
    calib_df = pd.DataFrame(
        [
            {
                "Nominal Coverage": "68%",
                "Empirical Coverage (60-day backtest)": f"{within_68:.1f}%",
                "Well-Calibrated?": "✅ Yes" if abs(within_68 - 68) <= 8 else "⚠ Off",
            },
            {
                "Nominal Coverage": "90%",
                "Empirical Coverage (60-day backtest)": f"{within_90:.1f}%",
                "Well-Calibrated?": "✅ Yes" if abs(within_90 - 90) <= 8 else "⚠ Off",
            },
            {
                "Nominal Coverage": "95%",
                "Empirical Coverage (60-day backtest)": f"{within_95:.1f}%",
    ...
    section_header(
        "🔁", "Walk-Forward Backtest — Multi-Origin Validation", tag="Next-Level Rigor"
    )
    ...
    with st.spinner(
        f"Running walk-forward backtest across ~40 historical origins (this validates the fan chart's assumptions)..."
    ):
        wf_errors, n_origins_used = run_walk_forward_backtest(
            df, FEATURES, rf_model, horizon
        )

    if n_origins_used < 5:
        st.info(
            "⚠ Not enough historical data for a reliable walk-forward backtest at this forecast horizon. Try a shorter horizon in the sidebar."
        )
    else:
        ...
        fig_wf.add_trace(
            go.Scatter(
                ...
                line=dict(color="#B31942", width=2.5),
                marker=dict(size=4),
            )
        )
        fig_wf.add_trace(
        ...
        fig_wf.update_layout(
            title=dict(
                text=f"Empirical vs Theoretical Error Growth ({n_origins_used} Backtest Origins)",
                font=dict(size=13, color="#112E51"),
                x=0.01,
            ),
            xaxis_title="Days Ahead",
            yaxis_title="Error Std Dev (children)",
        )
        fig_wf = chart_layout(fig_wf, height=CHART_HEIGHT)

        gap_at_end = (
            float(empirical_std[-1] / theoretical_std[-1])
            if theoretical_std[-1] > 0
            else 1.0
        )
        verdict = (
            "underestimates"
            if gap_at_end > 1.15
            else "overestimates" if gap_at_end < 0.85 else "reasonably matches"
        )
        ...
        chart_row(
            ...
            [
                (
                    "Backtest Origins Used",
                    f"{n_origins_used}",
                    "Independent historical starting points",
                ),
                ("Day 1 Error (Empirical)", f"±{empirical_std[0]:,.0f}", None),
                (
                    f"Day {horizon} Error (Empirical)",
                    f"±{empirical_std[-1]:,.0f}",
                    "Real, not assumed",
                ),
                (
                    f"Day {horizon} Error (√h Rule)",
                    f"±{theoretical_std[-1]:,.0f}",
                    "What the fan chart assumes",
                ),
            ],
        ...
        wf_calib_df = pd.DataFrame(
            [
                {
                    "Nominal Coverage": "68%",
                    "Walk-Forward Empirical Coverage": f"{wf_cov_68:.1f}%",
                    "Well-Calibrated?": (
                        "✅ Yes" if abs(wf_cov_68 - 68) <= 10 else "⚠ Off"
                    ),
                },
                {
                    "Nominal Coverage": "90%",
                    "Walk-Forward Empirical Coverage": f"{wf_cov_90:.1f}%",
                    "Well-Calibrated?": (
                        "✅ Yes" if abs(wf_cov_90 - 90) <= 10 else "⚠ Off"
                    ),
                },
                {
                    "Nominal Coverage": "95%",
                    "Walk-Forward Empirical Coverage": f"{wf_cov_95:.1f}%",
                    "Well-Calibrated?": (
                        "✅ Yes" if abs(wf_cov_95 - 95) <= 10 else "⚠ Off"
                    ),
                },
            ]
        )
...
# =========================================================
# WHOLE DATASET — KEY FINDINGS
# =========================================================

# These findings describe the overall HHS dataset and forecast system,
# not a user-filtered subset.

# ── 1. Care-load forecast finding ─────────────────────────
care_forecast_direction = (
    "an increasing"
    if future_forecast.iloc[-1] > future_forecast.iloc[0]
    else "a decreasing"
    if future_forecast.iloc[-1] < future_forecast.iloc[0]
...
care_peak_status = (
    f"above the configured capacity threshold of {capacity_threshold:,} children"
    if forecast_peak > capacity_threshold
    else f"below the configured capacity threshold of {capacity_threshold:,} children"
)

# ── 2. Discharge-demand finding ───────────────────────────
discharge_gap = fc_avg_transfer - fc_avg_discharge

if discharge_gap > 0:
    discharge_outlook = (
        f"forecast intake exceeds forecast discharge by "
        f"{discharge_gap:,.1f} children/day"
    )
elif discharge_gap < 0:
    discharge_outlook = (
        f"forecast discharge exceeds forecast intake by "
        f"{abs(discharge_gap):,.1f} children/day"
    )
else:
    discharge_outlook = "forecast intake and discharge are broadly balanced"

# ── 3. Model comparison finding ───────────────────────────
best_model_row = results_df.loc[results_df["RMSE"].idxmin()]
worst_model_row = results_df.loc[results_df["RMSE"].idxmax()]

best_model_rmse = float(best_model_row["RMSE"])
best_model_mae = float(best_model_row["MAE"])
best_model_accuracy = float(best_model_row["Accuracy (%)"])

worst_model_rmse = float(worst_model_row["RMSE"])

# ── 4. Early-warning finding ──────────────────────────────
if surge_days is None:
    early_warning_text = (
        f"No capacity-threshold breach is projected within the next "
        f"{horizon} days. The forecast remains below the "
        f"{capacity_threshold:,}-child threshold throughout the forecast window."
    )
else:
    early_warning_text = (
        f"The first projected capacity-threshold crossing occurs on "
        f"<strong>Day {surge_days + 1}</strong>. "
        f"{len(breach_days_all)} of {horizon} forecast days "
        f"({breach_pct}%) are above the threshold."
    )

# ── 5. Confidence & uncertainty finding ───────────────────
if within_90 >= 82:
    calibration_text = (
        f"The 90% uncertainty band covered {within_90:.1f}% of actual "
        f"values in the 60-day holdout, which is reasonably close to its "
        f"nominal coverage."
    )
elif within_90 >= 70:
    calibration_text = (
        f"The 90% uncertainty band covered {within_90:.1f}% of actual "
        f"values in the 60-day holdout, indicating moderate calibration "
        f"but some uncertainty remains."
    )
else:
    calibration_text = (
        f"The 90% uncertainty band covered {within_90:.1f}% of actual "
        f"values in the 60-day holdout, so the forecast intervals should "
...
section_header(
    "📋",
    "Key Findings — Whole Dataset",
    "Portfolio-Level Forecast Intelligence"
)

finding_box(
    "📋 Key Findings — HHS Care Load & Placement Intelligence",

    # ── Finding 1: Care Load Forecast ──────────────────────
    f""
    f"📈 <strong>1. Care Load Forecast:</strong> "
    f"The selected <strong>{selected_model_display}</strong> model projects "
    f"an average HHS care load of <strong>{forecast_avg:,} children/day</strong> "
    f"over the next <strong>{horizon} days</strong>, with a peak forecast of "
    f"<strong>{forecast_peak:,} children</strong>. "
    f"The forecast indicates {care_forecast_direction} trajectory, with the "
    f"peak remaining <strong>{care_peak_status}</strong>.<br><br>"

    # ── Finding 2: Discharge Demand Forecast ───────────────
    f"🏥 <strong>2. Discharge Demand Forecast:</strong> "
    f"Over the next <strong>{horizon} days</strong>, Holt-Winters forecasts "
    f"an average of <strong>{fc_avg_discharge:,.1f} discharges/day</strong> "
    f"against <strong>{fc_avg_transfer:,.1f} transfers/day</strong>. "
    f"This means {discharge_outlook}. "
    f"The projected net intake–discharge balance is "
    f"<strong>{fc_net_avg:+,.1f} children/day</strong>, with positive values "
    f"indicating potential care-load accumulation.<br><br>"

    # ── Finding 3: Model Comparison ────────────────────────
    f"🤖 <strong>3. Model Comparison:</strong> "
    f"Among the evaluated forecasting models, "
    f"<strong>{best_model_name}</strong> produced the lowest test-set RMSE "
    f"of <strong>{best_model_rmse:,.1f}</strong>, with MAE of "
    f"<strong>{best_model_mae:,.1f}</strong> children and forecast accuracy "
    f"of <strong>{best_model_accuracy:.1f}%</strong>. "
    f"The highest RMSE was recorded by <strong>{worst_model_row['Model']}</strong> "
    f"at <strong>{worst_model_rmse:,.1f}</strong>. "
    f"The comparison therefore provides an evidence-based basis for selecting "
    f"the forecasting model rather than relying on a single modelling approach.<br><br>"

    # ── Finding 4: Early Warning System ────────────────────
    f"⚠️ <strong>4. Early Warning System:</strong> "
    f"{early_warning_text} "
    f"The system continuously compares projected care load against the "
    f"<strong>{capacity_threshold:,}-child capacity threshold</strong>, "
    f"providing an advance indicator of potential placement and staffing "
    f"pressure before the threshold is reached.<br><br>"

    # ── Finding 5: Confidence & Uncertainty ────────────────
    f"🎯 <strong>5. Confidence & Uncertainty:</strong> "
    f"The Random Forest test residuals have a standard deviation of "
    f"approximately <strong>±{resid_std:,.0f} children</strong>. "
    f"The estimated 68% uncertainty interval widens from approximately "
    f"<strong>±{half_68[0]:,.0f}</strong> on Day 1 to "
    f"<strong>±{half_68[-1]:,.0f}</strong> on Day {horizon}, showing that "
    f"forecast uncertainty increases with the prediction horizon. "
    f"{calibration_text} "
    f"Forecast values should therefore be treated as planning estimates "
    f"rather than exact future outcomes."
)


hhs_executive_summary(
    current_care=current_hhs_care,
    avg_transfer_30=avg_transfer_30,
    avg_discharge_30=avg_discharge_30,
    net_balance_30=net_balance_30,
    pressure_word=pressure_word,
    forecast_avg=forecast_avg,
    forecast_peak=forecast_peak,
    horizon=horizon,
    capacity_threshold=capacity_threshold,
    breach_days=len(breach_days_all),
